...
class Fused_Gromov_Wasserstein_distance():
    ...
    def __init__(self, alpha=0.5, method='shortest_path', features_metric='sqeuclidean', max_iter=500, verbose=False,
                 amijo=True):  # remplacer method par distance_method
        self.method = method
    ...
    def calc_fgw(self, M, C1, C2, t1masses, t2masses):
        transpwgw, log = fgw.fgw_lp((1 - self.alpha) * M, C1, C2, t1masses, t2masses, 'square_loss', G0=None,
                                    alpha=self.alpha, verbose=self.verbose, amijo=self.amijo, log=True)
