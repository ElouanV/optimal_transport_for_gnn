...
class Tree:
    ...
    def sort_nodes(self, explainer, metric, valid=True):
        ...
        graph_value = [(g.graph,
                       ...
                       for g in nodes if g.emb is not None and len(g.graph) > 1 and (
                               not valid or explainer.rule_evaluator.activate(g.emb))]
    ...
    def compute_score(self, graph, emb=None):
        ...
        if not self.target_rule:
            ...
            if self.dataset == "ba2":
                ...
                score = (1 - self.prop_ratio) * (metric_value - self.real_ratio[0]) / (
                        self.real_ratio[1] - self.real_ratio[0]) \
                ...
                if d1 >= layer - 1:
                    for j in range(i + 1, len(nodes)):
                        ...
                        if not graph.has_edge(v,
                                              index_u) and d2 >= layer - 1 and self.rule_evaluator.get_layer() + 1 >= d2:
